OC.L10N.register(
    "news",
    {
    "News" : "Новости",
    "an article" : "статья",
    "Someone" : "Пользователь",
    "%1$s shared \"%2$s\" with you" : "%1$s предоставил(а) вам доступ к «%2$s»",
    "{user} shared \"{article}\" with you" : "{user} предоставил(а) вам доступ к \"{article}\"",
    "News feeds" : "Новостные ленты",
    "Unread articles" : "Непрочтённые статьи",
    "News folders" : "Каталог новостей",
    "News articles" : "Каталог статей",
    "Shared with me" : "Поделились с вами",
    "Untitled article" : "Статья без названия",
    "Background job mode for feed updates" : "Режим фонового выполнения задач для обновления ленты новостей",
    "Feed update method is correctly configured." : "Метод обновления ленты новостей настроен правильно.",
    "Ajax or webcron mode detected! Your feeds will not be updated!" : "Обнаружен режим планировщика задач Ajax или webcron! Ленты не будут обновляться!",
    "An RSS/Atom feed reader" : "Читалка RSS / Atom",
    "📰 A RSS/Atom Feed reader App for Nextcloud\n\n- 📲 Synchronize your feeds with multiple mobile or desktop [clients](https://nextcloud.github.io/news/clients/)\n- 🔄 Automatic updates of your news feeds\n- 🆓 Free and open source under AGPLv3, no ads or premium functions\n\n**System Cron is currently required for this app to work**\n\nRequirements can be found [here](https://nextcloud.github.io/news/install/#dependencies)\n\nThe Changelog is available [here](https://github.com/nextcloud/news/blob/master/CHANGELOG.md)\n\nCreate a [bug report](https://github.com/nextcloud/news/issues/new/choose)\n\nCreate a [feature request](https://github.com/nextcloud/news/discussions/new)\n\nReport a [feed issue](https://github.com/nextcloud/news/discussions/new)" : "📰 Приложение для чтения RSS/Atom для Nextcloud\n\n- 📲 Синхронизация ваших лент с несколькими мобильными или настольными [клиентами] (https://nextcloud.github.io/news/clients/)\n- 🔄 Автоматическое обновление ваших новостных лент\n- 🆓 Бесплатно и с открытым исходным кодом по AGPLv3, без рекламы и премиум-функций\n\n**Для работы этого приложения требуется системный Cron**.\n\nТребования можно найти [здесь](https://nextcloud.github.io/news/install/#dependencies)\n\nСписок изменений доступен [здесь](https://github.com/nextcloud/news/blob/master/CHANGELOG.md)\n\nСоздать [отчёт об ошибке](https://github.com/nextcloud/news/issues/new/choose)\n\nСоздать [запрос новой функции](https://github.com/nextcloud/news/discussions/new)\n\nСообщить [о проблеме](https://github.com/nextcloud/news/discussions/new)",
    "Download" : "Скачать",
    "Close" : "Закрыть",
    "Internal server error!" : " Внутренняя ошибка сервера!",
    "Unknown error!" : "Неизвестная ошибка!",
    "Web address" : "Веб адрес",
    "Please check your data/nextcloud.log file for additional information!" : "Пожалуйста, проверьте файл data/nextcloud.log для получения дополнительной информации!",
    "No folder" : "Нет папки",
    "Folder name" : "Имя папки",
    "Username" : "Имя пользователя",
    "Password" : "Пароль",
    "Feed exists already!" : "Лента уже существует!",
    "New folder" : "Новая папка",
    "Folder exists already!" : "Папка уже существует!",
    "HTTP Basic Auth credentials must be stored unencrypted! Everyone with access to the server or database will be able to access them!" : "Аутентификационные данные HTTP Basic придётся сохранить в незашифрованном виде! Каждый, у кого есть доступ внутрь сервера или в базу данных, сможет прочитать их!",
    "Credentials" : "Учётные данные",
    "Auto discover Feed" : "Автообнаружение ленты",
    "Subscribe" : "Подписаться",
    "Successfully updated news configuration" : "Конфигурация новостей успешно обновлена",
    "Unable to update news config" : "Не удалось обновить конфигурацию новостей",
    "Logo purge has never been run." : "Очистка логотипов никогда не проводилась.",
    "Last logo purge ran {relativeLastLogoPurge}. Something is wrong." : "Последняя очистка логотипов выполнялась {relativeLastLogoPurge}. Что-то пошло не так.",
    "Last logo purge job ran {relativeLastLogoPurge}." : "Последнее задание по очистке логотипов выполнялось  {relativeLastLogoPurge}.",
    "No job execution data available. The cron job may not be running properly." : "Данные о выполнении задания отсутствуют. Возможно, задание cron выполняется некорректно.",
    "Last job execution ran {relativeTime}. Something is wrong." : "Последняя задача выполнялась {relativeTime}. Что-то пошло не так.",
    "Last job ran {relativeTime}." : "Последнее задание было запущено {relativeTime}.",
    "Maximum read count per feed" : "Максимальное количество статей для ленты",
    "Maximum redirects" : "Максимум перенаправлений",
    "Feed fetcher timeout" : "Время ожидания для загрузчика ленты",
    "Explore Service URL" : "URL службы обзора",
    "Update interval" : "Интервал обновления",
    "Use system cron for updates" : "Для получения обновлений используйте системный cron",
    "Disable this if you use a custom updater." : "Отключите эту функцию, если вы используете пользовательскую программу обновления.",
    "Defines the maximum amount of articles that can be read per feed which will not be deleted by the cleanup job; if old articles reappear after being read, increase this value; negative values such as -1 will turn this feature off." : "Максимальное количество прочитанных статей для каждой ленты, которые не будут удалены при очистке; если старые статьи появляются снова после прочтения, то увеличьте это значение; отрицательное значения, например, -1, полностью отключает эту функцию.",
    "Delete unread articles automatically" : "Автоматическое удаление непрочитанных статей",
    "Enable this if you also want to delete unread articles." : "Включить, если вы хотите удалить непрочитанные статьи.",
    "How many redirects the feed fetcher should follow." : "Сколько перенаправлений должен обрабатывать загрузчик лент.",
    "Maximum number of seconds to wait for an RSS or Atom feed to load; if it takes longer the update will be aborted." : "Максимальное количество секунд ожидания загрузки ленты RSS или Atom; если обновление занимает больше времени, то оно будет отменено",
    "If provided, the URL of this service will be queried to display the feeds in the explore feed section. To fall back to the built in explore service, leave this input empty." : "Указанный URL будет опрашиваться для отображения лент в секции Обзор. Для использования встроенного сервиса, оставьте поле ввода пустым.",
    "Interval in seconds in which the feeds will be updated." : "Интервал обновления лент в секундах",
    "Use next update time for feed updates" : "Использовать следующее время обновления для обновлений ленты",
    "Enable this to use the calculated next update time for feed updates. Disable to update feeds based solely on the update interval." : "Включите эту опцию, чтобы при обновлении каналов использовалось рассчитанное время следующего обновления. Отключите эту опцию, чтобы обновление каналов происходило исключительно в соответствии с интервалом обновления.",
    "Article list" : "Список статей",
    "Article details" : "Сведения о статье",
    "No article selected" : "Статья не выбрана",
    "Please select an article from the list." : "Пожалуйста, выберите статью из списка.",
    "Show all articles" : "Показать все статьи",
    "Move feeds" : "Перенести ленты",
    "Move feed" : "Перенести ленту",
    "Moving feeds…" : "Перемещение лент...",
    "Unable to move feed. Please try again later or check your connection." : "Не удалось перенести ленту. Пожалуйста, попробуйте позже или проверьте подключение.",
    "Some selected feeds could not be moved. Please try again later or check your connection." : "Некоторые выбранные ленты не удалось переместить. Пожалуйста, попробуйте позже или проверьте подключение.",
    "Please select the new folder" : "Пожалуйста, выберите новую папку",
    "Move" : "Переместить",
    "User Name" : "Имя пользователя",
    "Share" : "Поделиться",
    "Share with" : "Поделиться",
    "Share with {num} users" : "Поделиться с {num} пользователями",
    "Rename Folder" : "Переименовать Папку",
    "Are you sure you want to delete?" : "Вы уверены, что хотите удалить?",
    "Unable to save starred open state" : "Не удалось сохранить открытое состояние «избранного»",
    "Mark read" : "Пометить прочтённым",
    "All articles" : "Все статьи",
    "Recently viewed" : "Недавно просмотрено",
    "Starred" : "Избранное",
    "Has feeds with errors!" : "Имеются ленты с ошибками!",
    "Explore" : "Обзор",
    "Feed settings" : "Настройки ленты",
    "News settings" : "Настройки новостей",
    "Rename" : "Переименовать",
    "Delete" : "Удалить",
    "Rename Feed" : "Переименовать Новостную ленту",
    "Open Feed URL" : "Открыть URL канала",
    "Unpin from top" : "Открепить сверу",
    "Pin to top" : "Прикрепить вверху",
    "Newest first" : "Новые-старые",
    "Oldest first" : "Старые-новые",
    "Default order" : "Порядок по умолчанию",
    "Downloading article text" : "Получение текста статьи",
    "Downloading of the article text has failed: {errorMessage}" : "Получение текста статьи завершилось с ошибкой: {errorMessage}",
    "Show external media" : "Показать внешние медиа",
    "from" : "С",
    "External media loaded from" : "Внешние медиа загружены из",
    "Previous Item" : "Предыдущий элемент",
    "Next Item" : "Следующий элемент",
    "Undo article text replace" : "Отменить замену текста статьи",
    "Download web version of this article" : "Скачать веб-версию этой статьи",
    "Share within Instance" : "Поделиться в пределах экземпляра",
    "Toggle star article" : "Добавить/убрать из избранного",
    "Mark unread" : "Пометить как непрочитанное",
    "Close details" : "Закрыть подробные сведения",
    "shared by" : "поделился",
    "by" : "автор",
    "Play audio" : "Проиграть аудио-запись",
    "Download audio" : "Скачать аудио-запись",
    "Download video" : "Скачать видео",
    "Refresh list" : "Обновить список",
    "Open website" : "Открыть веб-сайт",
    "Keep article unread" : "Оставить статью непрочитанной",
    "Keep article unread (auto-filtered)" : "Оставить статью непрочитанной (авто-фильтр)",
    "Toggle keep current article unread" : "Переключатель пометки текущей статьи непрочитанной",
    "Remove keep article unread" : "Удалить отметку от непрочтении",
    "Default" : "По умолчанию",
    "Compact" : "Компактный вид",
    "Screenreader" : "Чтение с экрана",
    "Vertical" : "По вертикали",
    "Horizontal" : "По горизонтали",
    "Off" : "Отключить",
    "Always" : "Всегда",
    "Ask" : "Спросить",
    "Never" : "Никогда",
    "Please select a valid OPML file" : "Выберите корректный OPML файл",
    "File successfully uploaded" : "Файл успешно загружен",
    "Error uploading the opml file" : "Ошибка при загрузке OPML файла",
    "Error connecting to the server" : "Ошибка при подключении к серверу",
    "Error retrieving the opml file" : "Ошибка при получении OPML файла",
    "Please select a valid json file" : "Выберите корректный json файл",
    "Error uploading the json file" : "Ошибка при загрузке json файла",
    "Error retrieving the json file" : "Ошибка при получении json файла",
    "General" : "Основные",
    "Disable mark read through scrolling" : "Отключить установку отметок о прочтении при прокрутке",
    "Reverse ordering (oldest on top)" : "Обратный порядок (самые старые сверху)",
    "Disable automatic refresh" : "Отключить автоматическое обновление",
    "Media" : "Медиа",
    "Show embedded thumbnails" : "Показать встроенные миниатюры",
    "Show images in the article text" : "Показывать изображения в тексте статьи",
    "Show iframes in the article text" : "Показывать iframe в тексте статьи",
    "Appearance" : "Внешний вид",
    "Display mode" : "Режим отображения",
    "Split mode" : "Раздельный режим",
    "Import" : "Импортировать",
    "Export" : "Экспорт",
    "Subscriptions (OPML)" : "Подписки (OPML)",
    "Importing feeds" : "Импортирование лент",
    "Please wait" : "Подождите",
    "Articles (JSON)" : "Статьи (JSON)",
    "Keyboard shortcuts" : "Горячие клавиши",
    "Refresh" : "Обновить",
    "Screenreader mode only" : "Только в режиме экранного считывателя",
    "Jump to previous article" : "Перейти на предыдущую статью",
    "Jump to next article" : "Перейти на следующую статью",
    "Item navigation and control" : "Навигация по элементам и управление ими",
    "Open article in new tab" : "Открыть статью в новой вкладке",
    "Show article details in compact view" : "Показать подробные сведения о статье в компактном виде",
    "Close article details in compact view" : "Скрыть подробные сведения о статье в компактном виде",
    "Download web version of the selected article" : "Скачать веб-версию выбранной статьи",
    "Feed/Folder navigation and control" : "Навигация и управление лентами/папками",
    "Jump to previous feed" : "Перейти на предыдущую ленту",
    "Jump to next feed" : "Перейти на следующую ленту",
    "Jump to previous folder" : "Перейти к предыдущей папке",
    "Jump to next folder" : "Перейти к следующей папке",
    "Mark current articles feed/folder as read" : "Пометить статьи текущей ленты/папки как прочитанные",
    "Resources" : "Ресурсы",
    "Unable to update keyword filters. Please try again." : "Не удалось обновить фильтры по ключевым словам. Попробуйте ещё раз.",
    "Are you sure you want to delete {count} selected feeds?" : "Вы уверены что хотите удалить {count} выбранных лент?",
    "Some selected feeds could not be deleted. Please try again later or check your connection." : "Некоторые выбранные ленты не удалось удалить. Пожалуйста, попробуйте позже или проверьте подключение.",
    "Last update" : "Последнее обновление",
    "Time when the feed was last downloaded or modified" : "Время последнего скачивания или редактирования ленты",
    "Next update" : "Следующее обновление",
    "Time when the next feed update will be done" : "Время следующего обновления ленты",
    "Articles per update" : "Статьи за обновление",
    "Maximum number of articles reached in a feed update" : "Достигнуто максимальное количество статей за обновление ленты",
    "Error Count" : "Счётчик ошибок",
    "Number of errors that have occurred since the last successful feed update" : "Количество ошибок, случившихся с момента последнего успешного обновления ленты",
    "Feed fetch options" : "Настройки получения обновлений ленты",
    "Feed update is enabled" : "Обновление ленты включено",
    "Feed update is disabled" : "Обновление ленты выключено",
    "Keep read status on update" : "Сохранять статус прочтения при обновлении",
    "Mark as unread on update" : "Отмечать как непрочитанное при обновлении",
    "Use article text provided by the feed" : "Используйте текст статьи, предоставленный лентой",
    "Loading feeds" : "Загрузка лент",
    "Keyword filters" : "Фильтр по ключевым словам",
    "Select all feeds" : "Выбрать все ленты",
    "Selected feeds" : "Выбранные ленты",
    "Move selected" : "Переместить выбранное",
    "Delete selected" : "Удалить выбранное",
    "Options" : "Варианты",
    "Title" : "Название",
    "Folder" : "Каталог",
    "Select feed {feed}" : "Выбрать ленту {feed}",
    "Enable feed update" : "Включить обновление ленты",
    "Disable feed update" : "Выключить обновление ленты",
    "Not available" : "Не доступен",
    "Save" : "Сохранить",
    "Clear" : "Очистить",
    "All Articles" : "Все статьи",
    "No feeds found to add" : "Не найдено ни одного канала для добавления",
    "Subscribe to" : "Подписаться",
    "Unread Articles" : "Непрочитанные статьи"
},
"nplurals=4; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<12 || n%100>14) ? 1 : n%10==0 || (n%10>=5 && n%10<=9) || (n%100>=11 && n%100<=14)? 2 : 3);");
