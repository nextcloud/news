...
describe('folder.ts', () => {
	...
	describe('actions', () => {
		it('FETCH_FOLDERS should call FolderService.fetchAllFolders and then commit folders returned to state', async () => {
			...
			FolderService.fetchAllFolders.mockResolvedValue({ data: { folders: [] } })
		...
		it('ADD_FOLDERS should call FolderService.createFolder and then commit the folders returned to state', async () => {
			...
			FolderService.createFolder.mockResolvedValue({ data: { folders: [] } })
		...
		it('FOLDER_SET_NAME should call FolderService.renameFolder and then commit deleted folder to state', async () => {
			...
			FolderService.renameFolder.mockResolvedValue()

			const folder = {}
			const commit = vi.fn()

			await actions[FOLDER_ACTION_TYPES.FOLDER_SET_NAME]({ commit }, { folder, name: 'newName' })
	...
	describe('mutations', () => {
		it('SET_FOLDERS should add the passed in folders to the state', () => {
			const state = { folders: [] } as AppState
			let folders = []
		...
		it('DELETE_FOLDER should remove the passed in folder from the state', () => {
			const state = { folders: [{ name: 'test' }] } as AppState
			const folders = [state.folders[0]]
		...
		it('SET_FEEDS should add the feed to the folder in the state', () => {
			const state = { folders: [{ name: 'test', id: 123 }] } as AppState
			const feeds = [{ id: 345, folderId: 123, title: 'article' }]
		...
		it('SET_FEEDS should update the folder unreadCount with the feed unreadCount', () => {
			const state = { folders: [{ name: 'test', id: 123, unreadCount: 0 }] } as AppState
			const feeds = [{ id: 345, folderId: 123, title: 'article', unreadCount: 5 }]
		...
		it('SET_FEEDS should update the folder updateErrorCount with the feed updateErrorCount when greater than eight', () => {
			const state = { folders: [{ name: 'test', id: 123, feedCount: 0 }] } as AppState
			const feeds = [{ id: 345, folderId: 123, title: 'article', updateErrorCount: 9 }]
		...
		it('ADD_FEED should add the feed to the folder in the state and update counters', () => {
			const state = { folders: [{ name: 'test', id: 123, feeds: [], feedCount: 0, updateErrorCount: 0 }] } as AppState
			const feed = { id: 345, folderId: 123, title: 'article', unreadCount: 5, updateErrorCount: 9 }
		...
		it('MODIFY_FOLDER_UNREAD_COUNT should update the folder feedCount in the state based on the delta', () => {
			const state = { folders: [{ name: 'test', id: 123, feedCount: 10 }] } as AppState
